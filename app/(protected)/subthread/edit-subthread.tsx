...
import {
  useFetchSubThreadByIdQuery,
  useUpdateSubThreadMutation,
} from "redux/api";

type EditSubThreadFormData = {
  title: string;
  description: string;
  labelColor: string;
  imageUrl: string;
...
const editSubThreadSchema = yup.object().shape({
...
export default function EditSubthreadScreen() {
  const navigation = useNavigation();
  const [showmodal, setShowmodal] = useState(false);
  const [selectedColor, setSelectedColor] = useState("");
  const [image, setImage] = useState<string | null>(null);
  const { subThreadId } = useLocalSearchParams<{ subThreadId: string }>();
  ...
  const {
    ...
    formState: { errors },
    setValue,
  } = useForm<EditSubThreadFormData>({
    resolver: yupResolver(editSubThreadSchema),
    defaultValues: {
      title: "",
      description: "",
      labelColor: "",
  ...
  const [updateSubThread] = useUpdateSubThreadMutation();

  useEffect(() => {
    if (subThreadResponse?.data?.subthread) {
      setValue("title", subThreadResponse.data.subthread.name);
      setValue("description", subThreadResponse.data.subthread.description);
      setValue("labelColor", subThreadResponse.data.subthread.label_color);
      setValue("imageUrl", subThreadResponse.data.subthread.image_url);
    }
  }, [subThreadResponse, setValue]);
  ...
  const backgroundColorStyle = useAnimatedStyle(() => ({
    backgroundColor: colorPickerValue.value,
  }));
  ...
  const handleEditSubthread = async (formData: EditSubThreadFormData) => {
    try {
      ...
      toast.showToastError("Error Updating Submeow", error);
    }
  };

  if (isFetchSubThreadByIDLoading) {
    return <Loading />;
  }

  if (fetchSubthreadByIDError) {
    return <Error />;
  }
      ...
      <View flex={1} padding="$3">
        <YStack gap="$3">
          ...
          <View>
            ...
            <Button
              bg={selectedColor ? selectedColor : "$backgroundSoft"}
              padding="$1"
              borderRadius="$3"
              onPress={() => setShowmodal(true)}
            >
              <Text color={"$primary"}>Choose Label Color</Text>
            </Button>
            {errors.labelColor && (
              <Text color="$red10" fontSize={12}>
          ...
          <Button
            onPress={handleSubmit(handleEditSubthread)}
            bg="$primary"
            color="white"
          >
            Update
